...
const Desserts: NextPage = () => {
	...
	const desserts = products.filter((val: any) => {
		return val;
	});
	...
	const getProducts = async () => {
		...
		const response = await fetch(
			`https://online-menu-pedro-fagundes.herokuapp.com/products/desserts-${i18n?.language}`
		);
	...
	const productShow = desserts.map(
